...
describe "when parsing formatted text" do     
  ...
  it "should raise TagError when it does not recognize a tag" do
    ...
    assert_raises(Prawn::Formatter::Parser::TagError) { parser.next }
  end
  ...
  it "should raise TagError when it a tag is closed with the wrong type" do
    ...
    assert_raises(Prawn::Formatter::Parser::TagError) { parse(parser) }
  end
  ...
  private
    ...
    def parser_for(text, opts={})
      ...
      @parser = Prawn::Formatter::Parser.new(@pdf, text, opts.merge(:styles => styles))
    end
